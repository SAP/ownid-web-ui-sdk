...
export default class InlineWidget extends BaseCommonComponent<InlineWidgetOptions> {
  ...
  protected render(options: InlineWidgetOptions): HTMLElement {
    ...
    const message = TranslationService.texts[options.lang].inline.message;
...
`;
    ...
    options.targetElement.parentNode!.insertBefore(element, options.targetElement.nextSibling);

    let [offsetX, offsetY] = options.offset || [0, 0];
