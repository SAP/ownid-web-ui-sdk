...
export default class Qr extends BaseCommonComponent<QrOptions> {
  ...
  protected render(options: QrOptions): HTMLElement {
    ...
    const title = options.title
      ? `<div class="ownid-title">${ options.title }</div>`
      : '';

    const subTitle = options.subtitle
      ? `<div class="ownid-subtitle">${ options.subtitle }</div>`
      : '';
...
.ownid-qr-code{margin-right:25px}
.ownid-qr-pane--titles{display:flex;flex-direction:column;justify-content:space-between;flex:1;}
