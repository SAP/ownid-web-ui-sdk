...
export default class Qr extends BaseCommonComponent<QrOptions> {
  ...
  protected render(options: QrOptions): HTMLElement {
    ...
    const title = options.title ? `<div style="font-family:'SF Compact Text',sans-serif;font-weight:bold;font-size:20px;line-height:32px;margin-bottom:4px;">${ options.title }</div>` : '';

    const subTitle = options.subtitle ? `<div style="font-family:'SF Compact Text',sans-serif;font-weight:500;font-size:16px;line-height:24px;">${ options.subtitle }</div>` : '';
    ...
    wrapper.innerHTML = `
      ...
      <div ownid-done style="display:none; position: absolute;top: 0;background: #FFF;width: 100%;height: 100%;justify-content: space-around;align-items: center;">
        ...
        <div style="font-weight: bold;font-size: 14px;line-height: 20px;color: #000000;">Skip the password enabled.<br>Finish registering...</div>
      </div>`;
